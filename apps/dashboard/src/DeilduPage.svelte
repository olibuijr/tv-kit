        ...
        <section class="category-section">
          ...
          <div class="item-grid">
            {#each entries as item (item.id)}
              <article>
                <div class="art"><svelte:component this={icon(item.mediaKind)} size={30}/><b class:ready={item.status === "ready"} class:active={item.status === "downloading" || item.status === "starting"}>{status(item)}</b></div>
