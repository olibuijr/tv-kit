<script lang="ts">
  ...
  import { browserClientId, tvServerUrl, tvServerWebSocketUrl, type DashboardContent, type HomeState, type RuvNewsArticle, type RuvProgram, type Station } from "../../../packages/protocol";
  ...
  const log = pino({ name: "tv-kit-dashboard", level: "trace", browser: { asObject: true } });
  const dashboardClientId = browserClientId("dashboard");
  ...
  async function refreshContent() {
    ...
    try {
		const next = await fetchDashboardContent(contentController.signal, state?.deilduCategoryId ?? 0, 1, undefined, state?.deilduShowId ?? "");
  ...
  onMount(() => {
    ...
    const connect = () => {
      ...
      clearTimeout(retry);
      const nextSocket = new WebSocket(tvServerWebSocketUrl(dashboardClientId));
      socket = nextSocket;
      log.debug({ clientId: dashboardClientId }, "ws connecting");
      nextSocket.onopen = () => {
        if (disposed || socket !== nextSocket) return;
        connected = true; lastMessage = Date.now(); log.info({ clientId: dashboardClientId }, "ws open"); void refreshContent();
      };
      nextSocket.onerror = () => log.warn({ clientId: dashboardClientId }, "ws error");
      nextSocket.onclose = ({ code, reason }) => {
        if (disposed || socket !== nextSocket) return;
        connected = false; log.warn({ clientId: dashboardClientId, code, reason }, "ws close; reconnect scheduled"); retry = window.setTimeout(connect, 1_500);
      };
      nextSocket.onmessage = ({ data }) => {
        ...
        if (message.type === "state") {
          ...
          state = message.state;
			if (state.deilduCategoryId !== previousCategoryId || state.deilduShowId !== previousShowId) void refreshContent();
...
{#if state}
<div class="shell" class:mpv-overlay={state.media.engine === "mpv" && state.media.fullscreen}>
  {#if state.power}
    <header>
      <div class="identity"><Home size={25}/><div><strong>TV Kit</strong><span>RÚV · Sarpurinn · Útvarp</span></div></div>
      <div class="date"><strong>{dateText}</strong><span>{solar?.location ?? "Akureyri"} · {solarTime(solar?.sunrise)}–{solarTime(solar?.sunset)}</span></div>
  ...
  {:else if state.view === "deildu"}
		<DeilduPage categories={content.deilduCategories} items={content.deilduItems} shows={content.deilduShows} show={content.deilduShow} scrape={content.deilduScrape} selectedCategoryId={state.deilduCategoryId}/>
  ...
  {:else}
    <main class="home-page">
      ...
      <section class="tee-times panel"><div class="section-title"><div><Flag size={19}/><h2>Lausir rástímar í dag</h2></div><span>{content.golfTeeTimes ? `${content.golfTeeTimes.course} · ${content.golfTeeTimes.slots.length}` : "Ekki tiltækt"}</span></div>{#if content.golfPerson}<div class="person-bookings"><strong>{content.golfPerson}</strong>{#if content.golfBookings.length}{#each content.golfBookings as booking}<span><b>{golfDate(booking.date)}</b>{booking.time}</span>{/each}{:else}<span>Enginn skráður rástími næstu daga</span>{/if}</div>{/if}{#if content.golfTeeTimes?.slots.length}<div class="tee-grid">{#each content.golfTeeTimes.slots.slice(0,18) as slot}<article><strong>{slot.time}</strong><span>{slot.openSeats === 1 ? "1 laust sæti" : `${slot.openSeats} laus sæti`}</span></article>{/each}</div>{#if content.golfTeeTimes.slots.length > 18}<p>+{content.golfTeeTimes.slots.length - 18} tímar síðar í dag</p>{/if}{:else}<div class="empty compact">{content.golfTeeTimes ? "Engir lausir rástímar eftir í dag." : "Ekki tókst að sækja rástíma."}</div>{/if}{#if content.golfTeeTimes?.stale}<small class="stale">Sýni síðustu þekktu gögn.</small>{/if}</section>
    </main>
  {/if}

  <ScheduledTaskNotice />
  {#if state.power && (state.playing || state.media.status === "loading")}<div class="player-layer" transition:fly={{ y: 96, duration: 220 }}><GlobalPlayer {state} {now} {command} {liveProgramme}/></div>{/if}
</div>
