      ...
      <section class="channel-focus-grid">
        {#each channels as item}
          <article class:active={state.media.id === `ruv-channel-${item.channel.slug}`} class="channel-focus">
            <div class="channel-art" class:has-image={Boolean(item.current?.watchFromStart?.image)} style={item.current?.watchFromStart?.image ? `background-image:url('${item.current.watchFromStart.image}')` : undefined}>
