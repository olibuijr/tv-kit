      ...
      <div class="episodes">
        {#each podcast.episodes as episode (episode.id)}
          <button class:active={activeMediaId === `podcast-${episode.id}`} on:click={() => command("podcast-play", episode.id, `Spila ${episode.title}`)}>
            <span class="play thumb">{#if episode.artworkUrl}<img src={episode.artworkUrl} alt=""/>{/if}<i><Play size={16} fill="currentColor"/></i></span>
...
<style>
  ...
  .episodes button{grid-template-columns:108px 1fr}.thumb{position:relative;width:108px;height:72px;border-radius:8px;overflow:hidden;background:var(--raised);display:grid;place-items:center}.thumb img{width:100%;height:100%;object-fit:cover}.thumb i{position:absolute;width:30px;height:30px;border-radius:50%;display:grid;place-items:center;background:color-mix(in oklch,var(--primary),transparent 8%);color:white;box-shadow:0 3px 10px oklch(0 0 0/.35)}@media(max-width:760px){.episodes button{grid-template-columns:96px 1fr}.thumb{width:96px}}
</style>
