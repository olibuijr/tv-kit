<script lang="ts">
  ...
  type Schedule = { id: number; name: string; kind: "prompt" | "script"; task: string; timing: "manual" | "once" | "interval"; when: string; active: boolean; lastRun?: string; nextIn?: number; progress?: number; total?: number; running?: boolean };
  ...
  onDestroy(() => { if (progressTimer) clearInterval(progressTimer); if (countdownTimer) clearInterval(countdownTimer); });
</script>
