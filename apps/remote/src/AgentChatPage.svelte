<script lang="ts">
  ...
  type Schedule = { id: number; name: string; kind: "prompt" | "script"; task: string; timing: "manual" | "once" | "interval"; when: string; active: boolean; lastRun?: string; progress?: number; total?: number; running?: boolean };
  ...
  let schedules: Schedule[] = [{
    ...
    kind: "prompt",
    task: "Skanna 403 skrár og nota staðbundna LLM-líkanið á Titan í lotum til að hreinsa skráarheiti.",
    timing: "manual",
  ...
  function runSchedule(id: number) {
  ...
  onDestroy(() => { if (progressTimer) clearInterval(progressTimer); });
</script>
