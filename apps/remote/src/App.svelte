...
<style>
  ...
  @media(max-width:760px){header{padding-inline:12px;grid-template-columns:1fr auto 46px}header time{display:none}.connection{font-size:0}.connection :global(svg){display:block}.now-playing{left:8px;right:8px;padding:7px 9px;grid-template-columns:54px minmax(0,1fr) auto;gap:8px}.poster{width:54px;height:46px}.track h1{font-size:14px}.track p,.timeline,.times,.track>span{display:none}.transport{gap:3px}.transport button{width:36px;height:36px}.transport .primary{width:40px;height:40px}.transport button:first-child,.transport button:nth-child(3){display:none}.media-tools{grid-template-columns:repeat(3,1fr)}.live-grid,.remote-grid,.news-grid{grid-template-columns:1fr}.radio-grid,.program-grid{grid-template-columns:repeat(2,1fr)}nav button{width:auto;flex:1}.controls{grid-template-columns:62px 1fr 62px}.rocker{height:170px}}
