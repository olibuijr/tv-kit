<script lang="ts">
  ...
  onMount(() => {
    ...
    return () => { clearInterval(clockTimer); clearInterval(contentTimer); clearInterval(stationTimer); clearInterval(heartbeatTimer); clearTimeout(retry); cancelAnimationFrame(scrollFrame); contentController?.abort(); socket?.close(); stopSolar(); document.removeEventListener("visibilitychange", visibility); };
  });
</script>
