<script lang="ts">
  ...
  $: if (activeTab === "Sarpur" && state?.mediaProgramId && !selectedProgram && !detailLoading) command("media-program", 0, "Sarpurinn");
  ...
  function openTab(tab: Tab) {
    ...
    window.scrollTo(0, 0);
    const view = tab === "Sjónvarp" ? "tv" : tab === "Útvarp" ? "radio" : tab === "Hlaðvörp" ? "podcasts" : tab === "Deildu" ? "deildu" : tab === "Sarpur" ? "media" : tab === "Fréttir" ? "news" : "home";
    if (tab !== "Spjall") command("view", view, tab);
    if (tab === "Fréttir") command("news-scroll", 0);
    if (tab === "Sarpur") command("media-program", 0);
...
<div class="app-shell" class:chat-active={activeTab === "Spjall"}>
  ...
  <main class:chat-mode={activeTab === "Spjall"}>
    ...
    <section class="transport-settings">
      <div class="seek-control">
        <button aria-label="Spóla 15 sekúndur til baka" disabled={state.media.live || state.media.duration <= 0} on:click={() => command("seek", Math.max(0, state.media.currentTime - 15), "Spólað til baka")}><RotateCcw size={19}/></button>
        <input aria-label="Staða spilunar" type="range" min="0" max={Math.max(1, state.media.duration)} value={Math.min(state.media.currentTime, state.media.duration)} disabled={state.media.live || state.media.duration <= 0} on:change={(event) => command("seek", Number(event.currentTarget.value), "Spólað")}/>
        <button aria-label="Spóla 30 sekúndur áfram" disabled={state.media.live || state.media.duration <= 0} on:click={() => command("seek", Math.min(state.media.duration, state.media.currentTime + 30), "Spólað áfram")}><SkipForward size={19}/></button>
      </div>
      <div class="volume-control">
        <button aria-label={state.muted ? "Kveikja á hljóði" : "Þagga hljóð"} class:active={state.muted} on:click={() => command("toggle-mute", undefined, state.muted ? "Kveikt á hljóði" : "Þaggað")}>{#if state.muted}<SpeakerX size={19}/>{:else}<Speaker size={19}/>{/if}</button>
        <button aria-label="Lækka hljóð" on:click={() => command("volume", Math.max(0, state.volume - 5), "Hljóðstyrkur")}><Minus size={19}/></button>
        <output aria-live="polite">{state.muted ? "Þaggað" : `${state.volume}%`}</output>
        <button aria-label="Hækka hljóð" on:click={() => command("volume", Math.min(100, state.volume + 5), "Hljóðstyrkur")}><Plus size={19}/></button>
        <button class="stop" aria-label="Stöðva spilun" disabled={!state.media.src} on:click={() => command("stop-playback", undefined, "Spilun stöðvuð")}>Stöðva</button>
      </div>
    </section>
...
{#if friendDetail}<!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions --><div class="detail-overlay" on:click={closeFriendDetail}><!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions --><div class="detail-sheet" on:click|stopPropagation><div class="detail-header"><h3>{friendDetail.name}</h3><span class="detail-club">{friendDetail.club}</span><button class="detail-close" on:click={closeFriendDetail}><Minus size={20}/></button></div><div class="detail-stats"><span><b>Forgjöf</b><strong>{friendDetail.handicap ?? "—"}</strong></span><span><b>Meðlimanúmer</b><strong>{friendDetail.memberNumber || "—"}</strong></span></div></div></div>{/if}
    ...
    {:else if activeTab === "Sjónvarp"}
      <TvPage channels={channels} favoriteSlugs={state.tvFavorites} activeMediaId={state.media.id} {now} {command}/>
    ...
    {:else if activeTab === "Deildu"}
		<DeilduPage categories={content.deilduCategories} shows={content.deilduShows} show={content.deilduShow} scrape={content.deilduScrape} selectedCategoryId={state.deilduCategoryId} loading={deilduLoading} tmdbToday={content.tmdbToday} {command}/>
  ...
  <nav aria-label="Aðalvalmynd">{#each navTabs as tab}<button class:active={activeTab===tab.name} on:click={() => openTab(tab.name)}><svelte:component this={tab.icon} size={20}/><span>{tab.name}</span></button>{/each}</nav>
  <ScheduledTaskNotice />
...
<style>
  .transport-settings{display:grid;gap:9px;margin-top:10px}.seek-control{display:grid;grid-template-columns:48px 1fr 48px;align-items:center;gap:8px}.seek-control button,.volume-control button{height:44px;border:1px solid var(--border);border-radius:9px;background:var(--surface);display:grid;place-items:center}.seek-control input{width:100%;accent-color:var(--primary)}.volume-control{display:grid;grid-template-columns:48px 48px 1fr 48px minmax(72px,auto);align-items:center;gap:8px}.volume-control output{text-align:center;font-size:12px;font-variant-numeric:tabular-nums}.volume-control button.active{border-color:var(--primary);color:var(--primary)}.volume-control .stop{padding:0 12px;color:var(--primary);font-size:10px;font-weight:800}.transport-settings button:disabled{opacity:.35}
  ...
  nav{height:76px;padding-bottom:env(safe-area-inset-bottom);position:fixed;left:0;right:0;bottom:0;z-index:30;display:flex;justify-content:center;background:var(--header);border-top:1px solid var(--border);box-shadow:0 -8px 22px oklch(0 0 0/.22)}nav button{flex:1;min-width:0;max-width:120px;border:0;background:none;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;color:var(--muted);font-size:9px}nav button.active{color:var(--primary)}.toast{position:fixed;right:16px;bottom:88px;z-index:40;max-width:min(360px,calc(100vw - 32px));padding:10px 10px 10px 15px;border-radius:12px;background:oklch(.92 0 0);color:oklch(.16 0 0);font-size:11px;font-weight:700;display:flex;align-items:center;gap:10px;box-shadow:var(--shadow-card)}.toast button{width:28px;height:28px;border:0;border-radius:8px;background:oklch(.82 0 0);color:inherit}.loading{min-height:100dvh;display:flex;flex-direction:column;justify-content:center;gap:8px}button:focus-visible,input:focus-visible{outline:3px solid var(--primary);outline-offset:2px}
  /* Match the native frame's neutral macOS graphite materials. */
  .app-shell{--bg:oklch(.115 0 0);--header:color-mix(in oklch,oklch(.16 0 0) 88%,transparent);--surface:oklch(.18 0 0);--raised:oklch(.23 0 0);--border:oklch(1 0 0/.10);--ink:oklch(.96 0 0);--muted:oklch(.71 0 0);--primary:oklch(.67 .16 250);--presence-accent:oklch(.69 .15 155);--shadow-card:0 8px 24px oklch(0 0 0/.25);--shadow-soft:0 3px 12px oklch(0 0 0/.18)}
  nav{backdrop-filter:blur(24px) saturate(130%);-webkit-backdrop-filter:blur(24px) saturate(130%)}
  .home-news{min-height:92px;padding:10px 13px;grid-template-columns:72px 1fr;gap:12px}
  .news-strip img{width:72px;height:72px;object-fit:contain;border-radius:8px;background:var(--raised)}
  @media(max-width:760px){.now-playing{left:8px;right:8px;padding:7px 9px;grid-template-columns:54px minmax(0,1fr) auto;gap:8px}.poster{width:54px;height:46px}.track h1{font-size:14px}.track p,.timeline,.times,.track>span{display:none}.transport{gap:3px}.transport button{width:36px;height:36px}.transport .primary{width:40px;height:40px}.transport button:first-child,.transport button:nth-child(3){display:none}.media-tools{grid-template-columns:repeat(3,1fr)}.player-details{bottom:calc(144px + env(safe-area-inset-bottom))}.live-grid,.news-grid,.home-facts{grid-template-columns:1fr}.tee-grid{grid-template-columns:repeat(3,1fr)}.radio-grid,.home-program-grid{grid-template-columns:repeat(2,1fr)}nav button{width:auto;flex:1}}
