...
<section class="deildu-browser panel" aria-busy={scrape.running}>
  <header class="panel-heading">
    ...
    <button disabled={scrape.running} on:click={() => command("deildu-scrape", undefined, "Samstilli Deildu")}><RefreshCw size={17} class:spinning={scrape.running}/>{scrape.running ? "Samstilli" : "Uppfæra"}</button>
  </header>
    ...
    <div class="items">
      {#each visibleItems as item (item.id)}
        <button disabled={!item.playable || busy(item)} aria-label={item.playable ? `Spila ${item.title}` : `${item.title} er ekki spilunarhæft`} on:click={() => command("deildu-play", item.id, `Spila af Deildu: ${item.title}`)}>
          <div class="art"><svelte:component this={icon(item.mediaKind)} size={26}/>{#if item.playable}<Play class="play" size={18} fill="currentColor"/>{/if}</div>
          ...
          {#if item.status === "ready"}<Download class="ready" size={17}/>{/if}
        </button>
      {/each}
    </div>
  {/if}
...
<style>
  .deildu-browser{overflow:visible}.panel-heading button{height:44px;padding:0 8px;border:0;background:none;display:flex;align-items:center;gap:5px;color:var(--primary);font-size:11px}.panel-heading button:disabled{opacity:.55}.scrape-state{min-height:54px;padding:10px 14px;display:grid;grid-template-columns:1fr minmax(130px,250px);align-items:center;gap:12px;border-bottom:1px solid var(--border);background:var(--raised)}.scrape-state.error{color:var(--primary)}.scrape-state div{display:flex;min-width:0;flex-direction:column}.scrape-state strong{font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.scrape-state span{font-size:8px;color:var(--muted);margin-top:3px}.scrape-state progress{width:100%;accent-color:var(--primary)}.search{height:44px;margin:10px 10px 0;padding:0 11px;display:flex;align-items:center;gap:7px;border:1px solid var(--border);border-radius:9px;background:var(--raised)}.search input{width:100%;border:0;outline:0;background:none;font-size:11px}.categories{padding:10px;display:flex;gap:7px;overflow-x:auto;scrollbar-width:thin}.categories button{flex:0 0 auto;min-width:92px;height:54px;padding:6px 9px;border:1px solid var(--border);border-radius:9px;background:var(--raised);display:grid;grid-template-columns:20px 1fr;align-items:center;text-align:left}.categories button.active{border-color:var(--primary);color:var(--primary)}.categories span{font-size:9px;white-space:nowrap}.categories small{grid-column:2;font-size:7px;color:var(--muted)}.items{padding:10px;display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.items>button{min-height:104px;padding:0;position:relative;display:grid;grid-template-columns:78px 1fr;align-items:stretch;border:1px solid var(--border);border-radius:10px;overflow:hidden;background:var(--raised);text-align:left}.items>button:disabled{opacity:.55;cursor:not-allowed}.art{position:relative;display:grid;place-items:center;background:linear-gradient(135deg,var(--surface),var(--header));color:var(--primary)}.play{position:absolute;right:7px;bottom:7px}.copy{padding:9px;display:flex;min-width:0;flex-direction:column}.copy strong{font-size:10px;line-height:1.3;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.copy span{font-size:8px;color:var(--muted);margin-top:4px}.copy small{display:flex;align-items:center;gap:4px;font-size:8px;color:var(--primary);margin-top:5px}.copy em{font-size:8px;color:var(--muted);font-style:normal;margin-top:auto}.copy i{height:3px;background:var(--border);margin-top:5px}.copy i b{display:block;height:100%;background:var(--primary)}.ready{position:absolute;right:7px;top:7px;color:var(--presence-accent)}.empty{min-height:180px;display:grid;place-items:center;color:var(--muted)}.spinning{animation:spin 1s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}
  @media(max-width:760px){.items{grid-template-columns:1fr}.scrape-state{grid-template-columns:1fr}.categories button{min-width:84px}}
