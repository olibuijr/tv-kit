...
import {
  ...
  cachedGolfBoxRounds,
  fetchGolfBoxBookings,
  fetchGolfBoxFriends,
  fetchGolfBoxRounds,
  golfboxScheduleIntervalMs,
  golfboxTaskState,
  runGolfBoxTask,
} from "./golfboxTask";
import { getGolfTeeTimes } from "./golfTeeTimes";
import {
  appendAgentChatTurn,
  getCache,
...
const state: HomeState = stopTransientPlaybackOnStartup(
  normalizeHomeState(storedState ?? defaultState, {
...
const clientSeen = new Map<ServerWebSocket<WebSocketData>, number>();
const clientsById = new Map<string, ServerWebSocket<WebSocketData>>();
const staleClientTimer = setInterval(() => {
  const cutoff = Date.now() - 30_000;
  for (const client of clients) {
    if ((clientSeen.get(client) ?? 0) >= cutoff) continue;
    log.warn(
      { clientId: client.data.clientId, lastSeen: clientSeen.get(client) },
      "ws stale client closed",
    );
    client.close(1001, "stale");
    clients.delete(client);
    clientSeen.delete(client);
    if (clientsById.get(client.data.clientId) === client)
      clientsById.delete(client.data.clientId);
  }
}, 10_000);
staleClientTimer.unref();
let castPrevious: {
  media: MediaItem;
  playing: boolean;
  view: HomeState["view"];
...
const persistDeilduPlayback = () => {
  const match = state.media.id.match(/^deildu-(\d+)$/);
  if (match)
    saveDeilduPlaybackPosition(
      Number(match[1]),
...
const broadcastDeilduContentRefresh = () => {
...
const server = Bun.serve<WebSocketData>({
  ...
  async fetch(req, server) {
    ...
    if (url.pathname === "/ws") {
      ...
      const requestedClient = url.searchParams.get("client") || "anonymous";
      const clientId = /^[a-z0-9-]{1,32}$/.test(requestedClient)
        ? requestedClient
        : "anonymous";
      log.debug(
        { clientId, origin: req.headers.get("origin") },
        "ws upgrade requested",
      );
      if (server.upgrade(req, { data: { clientId } })) return undefined;
      log.warn({ clientId }, "ws upgrade failed");
      return errorResponse(req, "WebSocket upgrade required", 400);
    }
    ...
    if (url.pathname === "/agent/tasks/deildu-import") {
      ...
      if (!scrapeState.running && !deilduCleanupState.running)
        void scrapeDeildu(
          config.deilduScrapePages,
          broadcast,
          broadcastDeilduContentRefresh,
        );
      return corsJson(
        req,
    ...
    if (url.pathname === "/agent/tasks/golfbox-sync") {
      if (req.method === "GET")
        return corsJson(
          req,
          { task: golfboxTaskState, data: cachedGolfBoxBookings() },
          0,
        );
      if (req.method !== "POST")
        return errorResponse(req, "method not allowed", 405);
      if (!golfboxTaskState.running) void runGolfBoxTask(broadcast);
      return corsJson(req, { accepted: true, task: golfboxTaskState }, 0, 202);
    }
    if (url.pathname === "/agent/chat") {
      ...
      try {
        ...
        appendAgentChatTurn(message, JSON.stringify(result.reply));
        return corsJson(
          req,
          {
    ...
    // On-demand live GolfBox bookings (no cache read); falls back to the last
    // good cache as stale if the live fetch fails.
    if (url.pathname === "/golfbox/bookings") {
      try {
        return corsJson(req, await fetchGolfBoxBookings(), 0);
      } catch (error) {
        const cached = cachedGolfBoxBookings();
        if (cached) return corsJson(req, { ...cached, stale: true }, 0);
        return corsJson(
          req,
          {
            error:
              error instanceof Error ? error.message : "GolfBox fetch failed",
          },
          0,
          502,
        );
    ...
    if (url.pathname === "/golfbox/rounds") {
      try {
        return corsJson(req, await fetchGolfBoxRounds(), 0);
      } catch (error) {
        const cached = cachedGolfBoxRounds();
        if (cached) return corsJson(req, { ...cached, stale: true }, 0);
        return corsJson(
          req,
          {
    ...
    if (url.pathname === "/golfbox/friends") {
      try {
        return corsJson(req, await fetchGolfBoxFriends(), 0);
      } catch (error) {
        const cached = cachedGolfBoxFriends();
        if (cached) return corsJson(req, cached, 0);
        return corsJson(
          req,
          {
            error:
    ...
    if (url.pathname === "/dashboard/content") {
      ...
      const deildu = listDeilduItems(
		...
		const deilduShows = deilduCategory
			? listDeilduShows(deilduCategory)
			: [];
		const deilduShowId = url.searchParams.get("deilduShow") ?? "";
		const deilduShow = deilduShowId && deilduCategory
			? getDeilduShow(deilduCategory, deilduShowId)
			: null;
      const golfTeeTimes = await getGolfTeeTimes(dateAt(0));
      const golfbox = cachedGolfBoxBookings();
      return corsJson(
        req,
        {
          ...dashboardContent(),
  ...
  websocket: {
    open(ws) {
      const previous = clientsById.get(ws.data.clientId);
      if (previous && previous !== ws) {
        log.warn(
          { clientId: ws.data.clientId },
          "ws replacing existing client",
        );
        previous.close(1001, "replaced");
      }
      clientsById.set(ws.data.clientId, ws);
      clients.add(ws);
      clientSeen.set(ws, Date.now());
      log.info(
        { clientId: ws.data.clientId, clients: clients.size },
        "ws open",
      );
    ...
    close(ws, code, reason) {
      clients.delete(ws);
      clientSeen.delete(ws);
      if (clientsById.get(ws.data.clientId) === ws)
        clientsById.delete(ws.data.clientId);
      log.info(
        { clientId: ws.data.clientId, code, reason, clients: clients.size },
        "ws close",
      );
    },
    message(ws, raw) {
      ...
      try {
        ...
        log.warn({ clientId: ws.data.clientId }, "ws invalid JSON message");
        return;
      }
      if (
        decoded &&
        ...
        log.trace({ clientId: ws.data.clientId }, "ws ping received");
      ...
      if (message.action === "deildu-category") {
		...
		if (message.action === "deildu-show") {
			if (message.value && (!state.deilduCategoryId || !getDeilduShow(state.deilduCategoryId, message.value))) return;
			state.previousView = state.view;
			state.view = "deildu";
			state.deilduShowId = message.value;
			state.lastAction = message.label || "Deildu-þáttur";
			broadcast();
			return;
		}
...
const golfboxStartTimer = config.golfboxSchedulerEnabled
  ? setTimeout(runGolfBoxIfDue, config.golfboxSchedulerStartDelayMs)
  : undefined;
golfboxStartTimer?.unref();
const golfboxSyncTimer = config.golfboxSchedulerEnabled
  ? setInterval(runGolfBoxIfDue, golfboxScheduleIntervalMs())
  : undefined;
golfboxSyncTimer?.unref();
