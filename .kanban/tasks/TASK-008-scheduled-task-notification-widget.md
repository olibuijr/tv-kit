...
- [x] A passive notification widget appears in the lower-right corner of both the TV dashboard and tablet remote while a scheduled task is running.
- [ ] The widget renders the same task card content and live progress shown by the existing scheduled-task UI.
- [x] The widget smoothly slides up when work starts and smoothly slides down when it finishes, including tasks already running when a client connects or reconnects.
...
- [x] Motion respects the client's reduced-motion preference.
...
Implemented as one shared notification component used by both clients. It polls the authoritative server task endpoint, deduplicates the active task, and uses reduced-motion-aware transitions. Runtime visual verification remains pending.
